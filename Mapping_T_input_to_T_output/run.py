...
run_df_generation = True #to generate new data, set to true
use_max_cap = False #to test with max cap on top of line failures -- to see if max cap impacts pmf -- set to true
############################################################################################################
...
if use_max_cap:
    ...
    for key in region_failure_pmf:
        num_failures = sum(list(key)[0:-2])#grab number of failures -- changed -- don't include last in list, that's max cap
        ...
        if (sum(region_failure_pmf[key]) != 0):
            ...
            normalized_key = tuple([float(i)/sum(list(key)[0:-2]) for i in list(key)[0:-2]]) #changed to cut off the last value
            f_and_c_separated_pmf[num_failures][max_cap][normalized_key] = pmf
            f_and_c_separated_failure_tracker[num_failures][max_cap][normalized_key] = tracker
    #print(f_separated_pmf[number_of_line_failures]) #show the input Ts and output pmfs for 2 line failures



    f_and_c_separated_mu_and_var = [[calculate_mean_and_variance(f_and_c_separated_pmf[i][c], f_and_c_separated_failure_tracker[i][c])
        for i in number_of_line_failures_list]
                              for c in unique_caps]#TODO: fix this to include cap
    number_of_failures = []

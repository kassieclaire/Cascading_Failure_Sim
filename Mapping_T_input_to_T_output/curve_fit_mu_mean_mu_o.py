...
for f in failure_vals:
    ...
    #find unique input variances for that F and iterate through these
    input_var_vals = separated_by_f_df['var_i'].unique()
    #iterate through the input variances
    for var_i in input_var_vals:
        #get the dataframe for only this failure value and input variance
        separated_by_f_and_var_df = separated_by_f_df[separated_by_f_df['var_i'] == var_i]
        #find input means for that F and input variance and iterate through these
        input_mean_vals = separated_by_f_and_var_df['mu_i']
